    ...
    for item in items:
        ...
        for word in words:
            if word.lower() in nameProcedure2.lower():
                ...
                customer = item.find("td", class_="row-customerName").get_text(
                    strip=True
                )
                ...
                numberEIS = item.find("td", class_="row-registrationNumber").get_text(
                    strip=True
                )
                ...
                dateLast = item.find("td", class_="row-endPublicationDate").get_text(
                    strip=True
                )

                # Тип процедуры
                typeProcedure = str(
                    item.find("td", class_="row-typeTorgsName").get_text(strip=True)
                )

                # Статус процедуры
                statusProcedure = item.find("td", class_="row-statusName").get_text(
                    strip=True
                )
                if (
                    statusProcedure == "Завершена"
                    or statusProcedure == "Заказчик отказался от проведения закупки"
                    or statusProcedure == "Итоговый протокол сформирован"
                    or statusProcedure == "Подведение итогов (Протокол не опубликован)"
                    or statusProcedure == "Закупка не состоялась"
                    or statusProcedure == "Закупка не состоялась"
                    or statusProcedure == "Заключение договора"
                    or statusProcedure == "Протокол опубликован"
                ):
                    continue
                else:
                    ...
                    try:
                        ...
                        try:
                            ajaxId0 = soup2.find(
                                "td", class_="viewDatanLotPriceRequest"
                            ).get("lotid")

                        except:
                            try:
                                ajaxId0 = soup2.find(
                                    "td", class_="viewDatanLotAuction"
                                ).get("lotid")

                            except:
                                try:
                                    ajaxId0 = soup2.find(
                                        "td", class_="viewDatanLotEShop"
                                    ).get("lotid")

                                except:
                                    try:
                                        ajaxId0 = soup2.find(
                                            "td", class_="viewDatanLotMlpdo"
                                        ).get("lotid")
                    ...
                    ajaxhref = (
                        "http://t1.torgi223.ru/includes/AuctionIS/ajax/viewLot.php"
                    )
...
# Создаём каталог
def parse():
    for URL in ["http://t1.torgi223.ru/registry/list/"]:
